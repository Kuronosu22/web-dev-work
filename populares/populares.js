...
async function fetchPopularNews() {
    try {
        ...
        const response = await fetch(`https://newsapi.org/v2/everything?q=${filter  || "tech"} AND tech&sortBy=popularity&language=pt&apiKey=${apiKey}`);
...
document.addEventListener('DOMContentLoaded', fetchPopularNews);
