...
async function fetchRelevancyNews() {
    try {
        document.getElementById('newsContainer').innerHTML = '';
        var filter = document.getElementById("searchBar").value.toLowerCase()
        const response = await fetch(`https://newsapi.org/v2/everything?q=${filter  || "tech"} AND tech&sortBy=relevancy&language=pt&apiKey=${apiKey}`);
...
document.addEventListener('DOMContentLoaded', fetchRelevancyNews);
